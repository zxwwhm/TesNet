...
def default_argument_parser():
    ...
    parser.add_argument("--duplicate", action="store_true",default=False,
                        help="duplicate") 
    parser.add_argument("--rpn_novel_pro", action="store_true",default=False,
                        help="duplicate")                 
    parser.add_argument(
        "--machine-rank",
        type=int,
...
def annotations_to_instances(annos, image_size):
    ...
    return target.to("cuda:0")
        
        
def readxml(anno_file, image_size):
    tree = ET.parse(anno_file)
    classnames = ['aeroplane', 'bicycle', 'boat', 'bottle', 'car', 'cat', 'chair',
        'diningtable', 'dog', 'horse', 'person', 'pottedplant', 'sheep',
        'train', 'tvmonitor', 'bird', 'bus', 'cow', 'motorbike', 'sofa']
    novel = ['bird', 'bus', 'cow', 'motorbike', 'sofa']
...
#     print('img:', image_size)
    return annotations_to_instances(instances, image_size)
class DefaultPredictor:
    ...
    @torch.no_grad()
    def __call__(self, original_image, path = None):
        """
        Args:
        ...
        predictions, proposals = self.model.inference([inputs])
#         proposals[0].proposal_boxes = proposals[0].proposal_boxes[0:4]

        if path is not None:
            instances = readxml("/home/jovyan/BY1706159/gitcode/few-shot-object-detection_2/datasets/VOC2007/Annotations/" + path.split(".")[0].split('/')[-1] + '.xml', proposals[0].image_size)
#             print(instances)
        pred_boxes = proposals[0].proposal_boxes
        pred_scores = proposals[0].objectness_logits
        
        
        gt = Instances(proposals[0].image_size)
        gt.pred_boxes = instances.gt_boxes
        gt.pred_classes = instances.gt_classes
#         print(len(gt))
        tag = False
        if len(gt)!=0:
            match_matrix = pairwise_iou(instances.gt_boxes, pred_boxes)
            match_matrix = torch.max(match_matrix, dim = 0)[0]
            sel_idx = match_matrix >=0.5

            pred_boxes = proposals[0].proposal_boxes[sel_idx]
            pred_scores = proposals[0].objectness_logits[sel_idx]
            proposal_pred = Instances(proposals[0].image_size)
            proposal_pred.pred_boxes = pred_boxes
            proposal_pred.pred_scores = pred_scores
#             proposal_pred = detector_postprocess(proposal_pred, height, width)
            if match_matrix.max()>=0.5:
                tag = True
#             print(match_matrix.max(),match_matrix.shape )
        else:
            proposal_pred = Instances(proposals[0].image_size)
            proposal_pred.pred_boxes = pred_boxes
            proposal_pred.pred_scores = pred_scores
...
#         proposal_pred = Instances(proposals[0].image_size, dict({"pred_boxes": pred_boxes, "pred_scores": pred_scores}))
        
#         proposals[0].remove("proposal_boxes")
#         proposals[0].remove("objectness_logits")
#         print(len(proposals[0]))
#         proposals[0].set("pred_boxes",  pred_boxes)
#         proposals[0].set("objectness_logits", objectness_logits)
#         print(proposal_pred.image_size, gt.image_size)
#         c = Instances.cat([proposal_pred, gt])
        
        return {"instances":proposal_pred}, {"instances":gt.to("cpu")}, tag  #predictions
