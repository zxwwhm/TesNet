...
def build_rpnnovel_head(cfg, input_shape):
    """
    Build an RPN head defined by `cfg.MODEL.RPN.HEAD_NAME`.
    """
    # name = cfg.MODEL.RPN.HEAD_NAME
    # return RPN_HEAD_REGISTRY.get(name)(cfg, input_shape)
    return StandardRPNHead_novel(cfg,input_shape)


@RPN_HEAD_REGISTRY.register()
class StandardRPNHead_novel(nn.Module):
    """
    ...
    def forward(self, features,feature_rot = None):
        """
        Args:
...
class RPN_novel(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.rpn_head = build_rpnnovel_head(cfg, [input_shape[f] for f in self.in_features])



    def forward(self, images, features, gt_instances=None , feature_rot = None):
        """
        Args:
        ...
        with torch.no_grad():
            ...
            proposals = find_top_rpn_proposals(
                ...
                images,
                self.nms_thresh,
                self.pre_nms_topk[self.training]*0.5,
                self.post_nms_topk[self.training]*0.5,
...
def build_novel_generator(cfg,input_shape):
    return RPN_novel(cfg,input_shape)