...
class FastRCNNOutputs(object):
    ...
    def __init__(
        self, box2box_transform, pred_class_logits,pred_proposal_deltas, proposals, smooth_l1_beta,box_label=None,calibration=False
    ):
    # 
        """
        Args:
        ...
        # self.batch_size=SOLVER.IMS_PER_BATCH
    ...
    def margin_loss(self):
        ...
        score_prob = F.softmax(self.pred_class_logits, 1)[:,1]
        
        
        gt_label=torch.cat((self.box_label.long(),self.gt_classes),0)
        score_label = gt_label.view(batch_size, -1).float()
        gt_map = 1 - (score_label.unsqueeze(1) == score_label.unsqueeze(-1)).float()
    ...
    def softmax_cross_entropy_loss(self):
        ...
        if self.calibration:
            gt_label=torch.cat((self.box_label.long(),self.gt_classes),0)
            return F.cross_entropy(self.pred_class_logits,gt_label, reduction="mean")
        else:
...
class FastRCNNOutputLayers(nn.Module):
    ...
    def forward(self, x,y):
        ...
        # scores=[]
        # for t in x:
        #     score = self.cls_score(torch.Tensor(t).cuda())
        # #     scores.append(score)
        if self.training and self.calibration:  
            pred=self.cls_score(x)
